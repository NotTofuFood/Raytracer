...
public:
	...
	float getX(Vector3 vec3) { return vec3.x; };
	float getY(Vector3 vec3) { return vec3.y; };
	float getZ(Vector3 vec3) { return vec3.z; };

	float getDotProduct(Vector3 v1, Vector3 v2) { return (v1.x*v2.x) + (v1.y*v2.y) + (v1.z * v2.z); }
	...
	Vector3 vecMultiply(Vector3 v1, Vector3 v2) {
		Vector3 new_vector;
